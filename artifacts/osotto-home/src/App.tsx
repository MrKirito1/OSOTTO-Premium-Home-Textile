...
function QuoteSection({ prefillProduct }: { prefillProduct: Product | null }) {
  ...
  const openWhatsApp = () => {
    ...
    const product = products.find((item) => item.id === value('product'));
    const message = ['Merhaba, OSOTTO ürünleri hakkında teklif almak istiyorum.', `Ad Soyad: ${value('name')}`, `Firma: ${value('company')}`, `E-posta: ${value('email')}`, `Telefon: ${value('phone')}`, `Ülke: ${value('country')}`, `Ürün: ${product?.name || value('product')}`, `Ağırlık: ${value('weight')}`, `Ölçü: 220x240 cm`, `Adet: ${value('quantity')}`, `Mesaj: ${value('message')}`].join('\n');
...
function DealerSection() {
  ...
  return <section id="bayimiz" className="bg-[#687358] px-5 py-24 text-[#f3eee6] md:px-10 md:py-32"><div className="mx-auto grid max-w-[1180px] gap-16 md:grid-cols-[.85fr_1.15fr] md:gap-28">
    <div className="reveal"><span className="font-mono-ui text-[10px] uppercase tracking-[.2em] text-[#d9e3c2]">05 / Bayimiz olun</span><h2 className="font-display mt-7 text-[clamp(3.5rem,6.5vw,7rem)] leading-[.82] tracking-[-.05em]">OSOTTO ile<br /><i className="font-normal text-[#d9e3c2]">büyüyün.</i></h2><p className="mt-9 max-w-[360px] text-[13px] leading-[1.9] text-[#f3eee6]/75">OSOTTO ürünlerini işletmenizde sunmak ve iş ortaklığımız hakkında bilgi almak için başvurun.</p><div className="mt-10 border-t border-[#f3eee6]/25 pt-5 font-mono-ui text-[9px] uppercase tracking-[.14em] text-[#f3eee6]/70">Seçili ürünler · Esnek çalışma · Doğrudan iletişim</div></div>
    <div className="reveal delay-1">{sent ? <div className="flex min-h-[520px] flex-col justify-center border-t border-[#f3eee6]/25"><Check size={28} /><h3 className="font-display mt-7 text-5xl">Başvurunuz<br /><i className="font-normal text-[#d9e3c2]">başarıyla alındı.</i></h3><p className="mt-5 text-[13px] text-[#f3eee6]/75">Ekibimiz başvurunuzu inceleyerek sizinle iletişime geçecektir.</p><button onClick={() => setSent(false)} className="mt-8 w-fit border-b border-[#f3eee6]/40 pb-1 font-mono-ui text-[10px] uppercase tracking-[.15em]" data-testid="button-new-dealer">Yeni başvuru</button></div> : <form ref={formRef} onSubmit={submit} className="border-t border-[#f3eee6]/25">
...
function Home() {
  ...
  useEffect(() => { document.title = 'OSOTTO | Premium Home Textile'; const description = 'OSOTTO premium ev tekstili ve battaniye koleksiyonları. Bursa’dan dünyaya uzanan kaliteli, modern ve iyi hissettiren dokular.'; let meta = document.querySelector('meta[name="description"]'); if (!meta) { meta = document.createElement('meta'); meta.setAttribute('name', 'description'); document.head.appendChild(meta); } meta.setAttribute('content', description); [['og:title', 'OSOTTO | Premium Home Textile'], ['og:description', description], ['og:type', 'website'], ['og:image', '/images/osotto-hero.jpg']].forEach(([property, content]) => { let tag = document.querySelector(`meta[property="${property}"]`); if (!tag) { tag = document.createElement('meta'); tag.setAttribute('property', property); document.head.appendChild(tag); } tag.setAttribute('content', content); }); }, []);
